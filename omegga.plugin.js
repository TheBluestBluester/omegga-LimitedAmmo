...
let pawnlist = []; // Reduces amounts of logs that are generated by getting the pawn on player joining.
...
class LimitedAmmo {
	...
	async tick() {
		...
		for(var pi in players) {
			...
			const foundpawn = pawnlist.find(x => x.pl === player.name);
			if(foundpawn == null) {
				continue;
			}
			const ppawn = foundpawn.pw
	...
	async initpawn(player) { // For some unknown to god reason when i put this in a function "await" is considered to be an unexpected identifier... Even though i need it because otherwise the promise doesnt finish in time...  ???????????????????????????
		const pl = await this.omegga.getPlayer(player.name); // Egg
		// Oh and btw if i try to put this directly into the join event without the timeout .getPlayer() will return undefined.
		const pawn = await pl.getPawn();
		pawnlist.push({pl: pl.name, pw: pawn});
	}
	...
	async init() {
		...
		this.omegga.on('join', async player => {
			if(pawnlist.find(x => x.pl === player.name) == null) {
				setTimeout(() => this.initpawn(player), 100);
			}
		})
		...
		for(var p in players) {
			...
			pawnlist.push({pl: player.name, pw: pawn});
		}
